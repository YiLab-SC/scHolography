...
#' @import dplyr
#' @import matchingR
scHolographyMapToPixel<-function(scHolography.obj,nCperP=NULL,seed=60611,fov=NULL, which.fov = NULL){

  scHolography.sc<-scHolography.obj$scHolography.sc


  if(is.null(nCperP)){
    nCperP = ceiling(ncol(scHolography.sc)/ncol(scHolography.obj$scHolography.sp))
  }

  iter.mean<-scHolography.obj$est.array
  diag.mean<-(iter.mean+t(iter.mean))/2
  cell.to.pix.dis<-diag.mean[(ncol(scHolography.obj$scHolography.sp)+1):(ncol(scHolography.obj$scHolography.sp)+ncol(scHolography.sc)),1:ncol(scHolography.obj$scHolography.sp)]
  ...
  if((1 %in% which.fov)){
    if (length(which.fov) > 1) {
      fov <- fov[1]
    }
    sp.coord <- ( SeuratObject::GetTissueCoordinates(scHolography.obj$scHolography.sp[[as.character(fov)]][["centroids"]]))[, c("x", "y")]
  }else{

    if("imagerow"%in%colnames(scHolography.obj$scHolography.sp@images[[1]]@coordinates)){
      sp.coord <- scHolography.obj$scHolography.sp@images[[1]]@coordinates[, c("imagerow","imagecol")]
    }else if("x"%in%colnames(scHolography.obj$scHolography.sp@images[[1]]@coordinates)){
      sp.coord <- scHolography.obj$scHolography.sp@images[[1]]@coordinates[, c("x","y")]
    }


  }

  amp.vec<-as.vector(apply(sp.coord,1, function(x){
