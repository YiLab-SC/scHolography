...
merfish.seurat.mouse1_sample1.slice21.sub@images$image =  new(
  Class = 'SlideSeq',
  assay = "Spatial",
  key = "image_",
  coordinates = merfish.seurat.mouse1_sample1.slice21.sub@meta.data[,c("CentroidX","CentroidY")]
)
```
...
sp.integrated.merfish.slice50toslice50 <- dataAlign(low.res.sp =  merfish.seurat.mouse1_sample1.slice50.sub_dup,high.res.sp =  merfish.seurat.mouse1_sample1.slice50.sub,scProcessed = T,stProcessed = T,future.size = 7000)
scHolography.obj.slice50toslice50<- trainHolography(sp.integrated.merfish.slice50toslice50)
saveRDS(scHolography.obj.slice50toslice50,"/projects/b1042/YiLab/Cady/Code_3rdDraft/Merfish_3D/paste_sample1//merfish.slice50toslice50.rds")
scHolographyPlot(scHolography.obj.slice50toslice50,color.by = "subclass")


def.col <- rep("black",length(unique(scHolography.obj.slice50toslice50$scHolography.sc$subclass)))
def.col[which(stringr::str_sort(unique(scHolography.obj.slice50toslice50$scHolography.sc$subclass),numeric = T)%in%stringr::str_sort(unique(scHolography.obj.slice50toslice50$scHolography.sc$subclass[which(scHolography.obj.slice50toslice50$scHolography.sc$class_label%in%"Glutamatergic")]),numeric = T))] <-(RColorBrewer::brewer.pal(12,"Paired"))[1:9] 




fig.2dq <- scHolographyPlot(scHolography.obj.slice50toslice50,color.by = "subclass",cells =which(scHolography.obj.slice50toslice50$scHolography.sc$class_label%in%"Glutamatergic") ,color = def.col)%>% plotly::layout(scene = list(camera = list(eye = list(x = 2.25, z = 0, y = 0))))
...
```{r}
merfish.seurat.mouse1_sample1.slice50.sub.glu.sub <- merfish.seurat.mouse1_sample1.slice50.sub[,which(merfish.seurat.mouse1_sample1.slice50.sub$class_label%in%"Glutamatergic")]

slice50 <-plot_ly(merfish.seurat.mouse1_sample1.slice50.sub.glu.sub@meta.data,x=~CentroidX,y=~-CentroidY,color=~subclass,colors = "Paired", marker = list(size = 3))%>% add_markers()
slice50
#plotly::orca(slice50, "slice50.svg",width = 4.5,height = 6)

#tiff(""/projects/b1042/YiLab/Cady/Code_3rdDraft/Merfish_3D/"/slice50.tiff", units="in", width=8, height=7, res=600)
ggplot(merfish.seurat.mouse1_sample1.slice50.sub.glu.sub@meta.data)+
...
```{r}
fig.3dq.layer <-scHolographyPlot(scHolography.obj.slice3Dtoslice50,color.by = "Inferred_Z",cells =which(scHolography.obj.slice3Dtoslice50$scHolography.sc$class_label%in%"Glutamatergic"),color = c('#e41a1c','#377eb8','#4daf4a','#984ea3','#FF7F00','#FFFF33'))%>% plotly::layout(scene = list(camera = list(eye = list(x = 1.6, z = 0.5, y = 0))))
fig.3dq.layer
...
```{r}
#tiff(""/projects/b1042/YiLab/Cady/Code_3rdDraft/Merfish_3D/"/Cortex.dist.2D.tiff", units="in", width=5, height=6, res=600)
scHolography::clusterDistanceBoxplot(scHolography.obj.slice50toslice50,annotationToUse = "subclass",query.cluster.list = c(
  "L2/3 IT","L4/5 IT","L5 IT","L6 IT"),reference.cluster = c("L6 IT","L6 IT Car3","L6 CT","L6b"))+ theme(legend.position = "none")+scale_fill_manual(values = color[c(1,2,4,7,8,3,5,6,9)])+theme_classic()+ 
  theme(axis.title.x = element_blank(),axis.text.x = element_text(angle = 45,hjust=1),axis.title.y = element_text(size = 16,face = "bold"),axis.text = element_text(size = 15,face = "bold"))+NoLegend()+ylab("SMN Distance to L6")+ 
  geom_signif(comparisons = my_comparisons,map_signif_level = TRUE,test.args = list(size=5),y_position = c(6,6.5,7))
...
sp.integrated.merfish.slice1toslice1 <- dataAlign(low.res.sp =  merfish.seurat.mouse1_sample1.slice1.sub_dup,high.res.sp =  merfish.seurat.mouse1_sample1.slice1.sub,scProcessed = T,stProcessed = T,future.size = 7000)
scHolography.obj.slice1toslice1<- trainHolography(sp.integrated.merfish.slice1toslice1)
saveRDS(scHolography.obj.slice1toslice1,"/projects/b1042/YiLab/Cady/Code_3rdDraft/Merfish_3D/paste_sample1//merfish.slice1toslice1.rds")
scHolographyPlot(scHolography.obj.slice1toslice1,color.by = "subclass")


def.col <- rep("black",length(unique(scHolography.obj.slice1toslice1$scHolography.sc$subclass)))
def.col[which(stringr::str_sort(unique(scHolography.obj.slice1toslice1$scHolography.sc$subclass),numeric = T)%in%stringr::str_sort(unique(scHolography.obj.slice1toslice1$scHolography.sc$subclass[which(scHolography.obj.slice1toslice1$scHolography.sc$class_label%in%"Glutamatergic")]),numeric = T))] <-(RColorBrewer::brewer.pal(12,"Paired"))[1:9] 




fig.2dq <- scHolographyPlot(scHolography.obj.slice1toslice1,color.by = "subclass",cells =which(scHolography.obj.slice1toslice1$scHolography.sc$class_label%in%"Glutamatergic") ,color = def.col)%>% plotly::layout(scene = list(camera = list(eye = list(x = 2.25, z = 0, y = 0))))
...
```{r}
merfish.seurat.mouse1_sample1.slice1.sub.glu.sub <- merfish.seurat.mouse1_sample1.slice1.sub[,which(merfish.seurat.mouse1_sample1.slice1.sub$class_label%in%"Glutamatergic")]

slice1 <-plot_ly(merfish.seurat.mouse1_sample1.slice1.sub.glu.sub@meta.data,x=~CentroidX,y=~-CentroidY,color=~subclass,colors = "Paired", marker = list(size = 3))%>% add_markers()
slice1
#plotly::orca(slice1, "slice1.svg",width = 4.5,height = 6)

#tiff(""/projects/b1042/YiLab/Cady/Code_3rdDraft/Merfish_3D/"/slice1.tiff", units="in", width=8, height=7, res=600)
ggplot(merfish.seurat.mouse1_sample1.slice1.sub.glu.sub@meta.data)+
  geom_point(aes(x=CentroidX,y = -CentroidY,color=subclass))+theme_void()+scale_color_brewer(palette = "Paired")+theme(axis.title = element_blank(),axis.text = element_blank(),axis.ticks = element_blank())+Seurat::NoLegend()
#dev.off()
```
...
```{r}
#tiff(""/projects/b1042/YiLab/Cady/Code_3rdDraft/Merfish_3D/"/Cortex.dist.2D.tiff", units="in", width=5, height=6, res=600)
scHolography::clusterDistanceBoxplot(scHolography.obj.slice1toslice1,annotationToUse = "subclass",query.cluster.list = c(
  "L2/3 IT","L4/5 IT","L5 IT","L6 IT"),reference.cluster = c("L6 IT","L6 IT Car3","L6 CT","L6b"))+ theme(legend.position = "none")+scale_fill_manual(values = color[c(1,2,4,7,8,3,5,6,9)])+theme_classic()+ 
  theme(axis.title.x = element_blank(),axis.text.x = element_text(angle = 45,hjust=1),axis.title.y = element_text(size = 16,face = "bold"),axis.text = element_text(size = 15,face = "bold"))+NoLegend()+ylab("SMN Distance to L6")+ 
  geom_signif(comparisons = my_comparisons,map_signif_level = TRUE,test.args = list(size=5),y_position = c(6,6.5,7))
